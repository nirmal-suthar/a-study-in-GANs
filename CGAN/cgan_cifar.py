...
# #parser.add_argument('--dataset', required=True, help='cifar10 | fashion-mnist | mnist')
...
# preprocessing for mnist, fashion-mnist, cifar10
if opt.dataset == 'mnist': 
    ...
    dataset = datasets.FashionMNIST(root = opt.dataroot, train=True,download=True, 
        transform=transforms.Compose([transforms.Resize(opt.imageSize), 
...
#     # a_loss = nn.DataParallel(a_loss)


# In[37]:
static_noise = (FT_a(np.random.normal(0, 1,(opt.batchSize, opt.latentdim)))).to(device)
static_gen_labels = (FT(np.random.randint(0, opt.n_classes, opt.batchSize))).to(device)


# training 
# mb = master_bar(range(opt.epoch))
for epoch in range(opt.epoch): 
    
    start = time.time()
    # for i, data in zip(progress_bar(range(100), parent=mb), dataloader):
    for i, data in enumerate(dataloader):
        ...
        gen_labels = Variable(FT(np.random.randint(0, opt.n_classes, batch_size))).to(device)
        ...
        if i%100 == 0: 
            vutils.save_image(gen_imgs, '%s/images/real_samples.png' % opt.output, normalize=True)
            static_fake = generator(static_noise, static_gen_labels)
            vutils.save_image(static_fake.detach(), '%s/images/fake_samples_epoch_%03d.png' % (opt.output, epoch), normalize=True)

        # mb.child.comment = f'Epoch {epoch} bar'

    print("[Epoch: %d/%d]" "[D loss: %f]" "[G loss: %f]" "[time(s): %f]" % (epoch+1, opt.epoch, d_loss.item(), g_loss.item(), (time.time()-start)))
    # mb.first_bar.comment = 'Training bar'

    # checkpoints 
    torch.save(generator.state_dict(), '%s/model/generator_epoch_%d.pth' % (opt.output, epoch))
    torch.save(discriminator.state_dict(), '%s/model/generator_epoch_%d.pth' % (opt.output, epoch))
